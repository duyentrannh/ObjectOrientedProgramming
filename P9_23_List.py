...
# And develop these methods using the list classes and then the dictionary classes.
...
class Country:
  ...
  # Now write a program that reads in a set of countries and print out the requirements
  
class CountryList: # Using list classes
    #Construct an empty list of countries
    def __init__(self):
        self._countries = []
    ...
    def addCountry(self, name, population, area):
        country = Country(name, population, area)
    ...
    def country_with_largest_area(self):
        ...
        for i in range(1,len(self._countries)):
            if self._countries[i].getArea() > largest.getArea():
    ...
    def country_with_largest_population(self):
        largest = self._countries[0]
        for i in range(1,len(self._countries)):
            if self._countries[i].getPopulation() > largest.getPopulation():
                largest = self._countries[i]
        return largest.toString()
    ...
    def country_with_largest_population_density(self):
        largest = self._countries[0]
        for i in range(1,len(self._countries)):
            if self._countries[i].getPopulationdensity() > largest.getPopulationdensity():
                largest = self._countries[i]
        return largest.toString()
...
def start_program():
    ...
    for _ in range(num_countries):
        country_list.getInput()

    #Print out the results
    print("Country with the largest area:", country_list.country_with_largest_area())
    print("Country with the largest population:", country_list.country_with_largest_population())
    print("Country with the largest population density:", country_list.country_with_largest_population_density())
