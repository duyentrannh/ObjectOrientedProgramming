...
import argparse
import textwrap
from P9_23_List import CountryList
...
def main():
    parser = argparse.ArgumentParser(prog='My test program',
                                    ...
                                    In addition, using list classes  to get the largest area, population and density.
                                    ...
                                    class CountryList: class uses methods to answer 3 questions

                                    1) addCountry : Add country to the lists
                                    @returnt the country list 
                                    ...
                                    country_list.addCountry('USA')   # add a country
                                    country_list.country_with_largest_area() # Get country with largest area
                                    country_list.country_with_largest_population() # Get country with largest population     
                                    country_list.country_with_largest_populationdensity() # Get country with largest population density
                                    country_list.getInput() # Get user input with valid values 
                                                                                                                                                                                                                                                                           
                                    ''')
                                    )
    parser.add_argument('--run_demo',required=True, action='store_true',help='Runs this demo')
...
def run_demo():
    ...
    countrylist.addCountry('USA',1000000,20.5)
    countrylist.addCountry('UK',800000,43.3)
    countrylist.addCountry('Norway',500000,13.3)
    print("Country with the largest area:" , countrylist.country_with_largest_area())

    print("Country with the largest population:",countrylist.country_with_largest_population())

    print("Country with the largest population density:",countrylist.country_with_largest_population_density())
